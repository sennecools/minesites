...
import { FONT_DISPLAY_NAMES, FONT_FAMILY_MAP } from '@/lib/theme-presets';
		...
		<div className="flex flex-col gap-0.5">
			{keys.map((key) => (
				<motion.button
					...
					className="w-full rounded px-1 py-2 text-left text-sm transition-colors hover:bg-zinc-100"
					style={{
						fontFamily: FONT_FAMILY_MAP[key],
						fontWeight: selected === key ? 700 : 400,
						borderBottom:
							selected === key ? `2px solid ${accentColor}` : '2px solid transparent',
