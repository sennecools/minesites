...
```typescript
// Before:
const server = await db.server.findUnique({ where: { id: serverId }, include: { sections: { orderBy: { order: "asc" } } } });
// After:
const website = await db.website.findUnique({ where: { id: serverId }, include: { sections: { orderBy: { order: "asc" } } } });
```
...
```typescript
// Before:
const existingServer = await db.server.findUnique({ where: { id: serverId }, select: { userId: true } });
// After:
const existingWebsite = await db.website.findUnique({ where: { id: serverId }, select: { userId: true } });
```
